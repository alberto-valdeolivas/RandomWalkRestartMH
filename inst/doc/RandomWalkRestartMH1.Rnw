...
to perform Random Walk with Restart on different types of networks. It is based
on the work we presented on the following article:
...
i) Monoplex, ii) Multiplex, iii) Heterogeneous and iv) Multiplex-Heterogeneous.
It is based on the work we presented in the article:
\url{https://www.biorxiv.org/content/early/2017/08/30/134734}.

RWR simulates an imaginary particle which starts on a seed(s) node(s) and
...
\item A multiplex network, which is a collection of layers (monoplex networks)
sharing the same nodes, but their edges represent relationships of
different nature in each layer \cite{Battiston2014}. In this case, the particle
can jump from one node to its counterparts on different layers.
...
connected thanks to bipartite edges, allowing the particle to jump between
the two networks.

\item A multiplex and heterogeneous network, which is built by linking the nodes
in every layer of the multiplex network to nodes of different nature thanks to
biparitte edges. The particle can now explore the full network.
\end{itemize}

The user can introduce up to four single networks (monoplex networks) to create
a multiplex network. The multiplex network can also be integrated, thanks to
biparite relationships, with a network containing nodes of different nature.
Proceeding this way a network, both multiplex and heterogeneous, will be
generated.

The user should take in account that this first version of the package is only
able to deal with undirected and unweighted networks. We hope to include these
new functionalities soon in an updated version of \Biocpkg{RandomWalkRestartMH}.
...
the computaiton time of this vignette.

The goal, as described in \cite{Valdeolivas2017}, is to
find candidate genes potentially associated with diseases by a
guilt-by-association approach. This is based on the fact that genes/proteins
associated with similar functions or similar phenotypes tend to lie closer in
biological networks.
...
results evolve due to the integration of additional and complementary networks.
...
so-called seed(s),
known to be implicated in a concrete function or in a specific disease, are
chosen as the starting point(s) of the algorithm. The walker explores the
neighbourhood of the seeds and the algorithm computes a score for all the nodes
of the network. The larger is the score of a node, the closer it is to the
seed(s).

Therefore, let us generate an object of the class \Rclass{Multiplex}, even if
it is a monoplex network, with our PPI network \cite{Valdeolivas2017}.
...
We need to define the seed(s) before running RWR on this PPI network. As
commented above, we are focusing on the example of the SHORT syndrome.
Therefore, we take the \textit{PIK3R1} gene as seed, and we execute RWR.
...
scored genes. Visualize the results in this way is always a good idea
in order to prioritize genes, since we can have a global view of all
the potential candidates. The results are presented in Figure~\ref{fig:1}.
...
we also use a heterogeneous network integrated by a PPI and a disease-disease
...
Let us generate a PPI-disease heterogeneous network. To do so, we load the
disease network and combine it with our \Rclass{Multiplex} object
containing the PPI thanks to the gene-diseases associations obtained from OMIM
\cite{Hamosh2005}. A \Rclass{MultiplexHet} object will be created, even if we
are dealing with a monoplex-heterogeneous network.
...
accounts for all the possible transitions of the walker within that network
\cite{Li2010}.
...
original features of the different sources.
...
compute a matrix that accounts for all the possible transitions of the walker
within this network \cite{Valdeolivas2017}.
...
\textit{PIK3R1} gehe and the SHORT syndrome disease (MIM code: 269880) as seeds
